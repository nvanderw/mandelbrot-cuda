#include <stdio.h>
extern "C" int *create_mandelbrot(int res_x, int res_y, float min_x, float min_y,
        float max_x, float max_y, int iter);

__global__ void mandelbrot(int *region, int offset, int2 res, float4 boundary, int iter);
__device__ int divergence(double2 c, int iteger);
...
extern "C" int *create_mandelbrot(int res_x, int res_y, float min_x, float min_y,
    ...
    size_t host_bytes = (size_t) size * sizeof(int);
    size_t job_bytes;
    size_t jobsize;
...
/* Tests the divergence of some given point a + b*i in the complex plane.
 *
 * Return value: if divergence was detected, the number of iterations in which
 * it occurred. Otherwise, iter is returned.
 */
__device__ int divergence(double2 c, int iter) {
    int i;
    double2 z = c;
    for(i=1;(i < iter) && (z.x < 2) && (z.y < 2);i++) {
...
__global__ void mandelbrot(int *region, int offset, int2 res, float4 boundary, int iter) {
    ...
    z.x = ((boundary.z - boundary.x) * (((double) pixel_x) + 0.5))/(res.x) + boundary.x;
    z.y = ((boundary.y - boundary.w) * (((double) pixel_y) + 0.5))/(res.y) + boundary.w;

    region[index] = divergence(z, iter);
}
